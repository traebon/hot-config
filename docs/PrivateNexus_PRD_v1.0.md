...
| AUTH-07 | MFA required for admin, superadmin, breakglass (Keycloak config) | BUILT — fixed 2026-07-23 (Keycloak `privatenexus` realm config, no app code). WebAuthn/passkey registration existed realm-wide but was never actually required. **Two separate enforcement points were needed, not one** — local password login and federated broker login (signing in via the linked `house-of-trae` account, the path real users including tristian actually use) go through entirely different Keycloak flows, and the first fix here only covered the local one. (1) Local path: duplicated the realm's `browser` flow to `browser - privileged MFA`, added 3 role-gated conditional sub-flows (admin/superadmin/breakglass) forcing `OTP Form` REQUIRED right after password entry, excluded those roles from the pre-existing optional-2FA fallback to avoid a double prompt, bound as the realm's `browserFlow`. (2) Broker path: built a second flow, `privatenexus post-broker MFA` (same 3 role-gated sub-flows), bound as the `house-of-trae` identity provider's `postBrokerLoginFlowAlias` — this runs on *every* broker login, unlike `firstBrokerLoginFlowAlias` which only fires once at initial account linking. Hit and fixed a real Keycloak gotcha here: a post-broker flow made *only* of conditional sub-flows throws `AuthenticationFlowException` (surfaces as "Invalid username or password") when none of the conditions match, because nothing in the flow ever executes — fixed by adding a REQUIRED `allow-access-authenticator` ("Allow access") as an unconditional pass-through fallback. Verified end-to-end on **both** paths using disposable test accounts (local test user for path 1; a real linked house-of-trae+privatenexus federated pair, driven through the actual multi-hop broker redirect chain with curl, for path 2) — role assigned → `CONFIGURE_TOTP`; role removed → normal flow, no MFA forced. All test accounts deleted after. **Operational note: the only current privileged-role holder (tristian, superadmin) has no MFA credential registered yet — his next `privatenexus.net` login (via either path) will stop and require TOTP enrollment (his choice, an authenticator app) before completing.** Scoped to the `privatenexus` realm only — no other realm or app affected. |
...
| ACT-03 | Maintenance mode — operator/admin, duration required, audit-logged | BUILT — fixed 2026-07-23 (`hot-privatenexus` `f495008`). Feature was already mature (auto-expiry timer, Grafana alert-silence integration, audit logging) but `duration` was optional server-side — omitting it left maintenance mode on indefinitely with no expiry and no alert suppression, invisible from the UI (frontend always sends a duration) but reachable via a direct API call. Now rejected with 400 if omitted. |
...
| FE-03 | Service detail: health history, backup summary, action buttons | BUILT, but was silently broken — the 2026-07-15 "verified present" check confirmed the JSX existed in source, not that it actually rendered. **Fixed 2026-07-24** (`hot-privatenexus` commit `c8115d7`): the Backup Records panel referenced an undefined `userRole` variable (leftover from before the `can(minRole)` helper existed), throwing an uncaught `ReferenceError` on every single service detail open, caught by the top-level ErrorBoundary as a generic "Something went wrong" screen. This affected 100% of services, not an edge case — found only by Mr. Byrne actually clicking through the real UI in a browser, not by any of the many source-level or curl/API-level audits that preceded it. **Lesson: "verified present in source" is not the same claim as "verified working" — future FE-xx audits of this app should include an actual browser render, not just a grep.** |
