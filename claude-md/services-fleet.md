...
| fleet-state-backup | systemd (`fleet-state-backup.timer`) | Not a container — nightly (03:10, `hot-config/scripts/`) systemd timer. Started 2026-09-06 scoped to sn-monitor only (the PBS-offsite-bridge resolution, `docs/HoT_PBS_Backup_Integration_Scope.md` Section 9 — PBS's real restore throughput measured ~1.5-2 MiB/s, 47+ hours for one 250GB VM, ruled out any full-image offsite bridge). **Widened fleet-wide the same day at Mr. Byrne's explicit direction** ("all key databases... email... I mean everything") after auditing every host for the same class of gap and finding a genuinely critical one: **Vaultwarden — every credential in this project — had zero backup coverage anywhere.** Now covers, per host: Gateway (Vaultwarden's SQLite DB, local `sqlite3 .backup`, no SSH needed); sn-monitor (Grafana Postgres DB, Uptime Kuma SQLite DB); sn-infra (Forgejo's Postgres DB + its real `git/repositories` data via a container-side `tar`, PowerDNS-Admin DB, hot-wiki DB, Namevault DB); hot-erp-nl (ERPNext's MariaDB dump + its real uploaded-file volumes, `sites-data`+`assets-data`); hot-pn (Nextcloud's Postgres DB + its real 13GB user-data directory, streamed via container-side `tar` straight into `gzip`, no local disk usage). All pushed directly to `hetzner-crypt`/`b2-hot-crypt`, same pipeline Keycloak/PrivateNexus's DB backups already use — no PBS/home-network dependency anywhere in this script. Docker-compose files, Caddyfile, and other static config are covered separately by `scripts/sync.sh`; real mail content (Maildir) by `backup-gateway-vps.sh` — this script is specifically the database/real-user-data layer neither of those touch. **First full run, 2026-09-07: all 12 pieces succeeded, zero failures, ~13GB total** (dominated almost entirely by Nextcloud's real data). Spot-verified restorable, not just pushed: Vaultwarden's copy passes `PRAGMA integrity_check` with its real user row intact; ERPNext/Nextcloud dumps have real valid schemas (929/131 `CREATE TABLE`+`INSERT` statements respectively); Forgejo's tar contains real `.git` objects (`tristian/privatenexus.git`), not just empty directories. **Real bug caught before trusting the very first version**: `push()` checked the logging `while`-loop's exit code instead of `rclone`'s own (via `PIPESTATUS`) — would have silently reported success on a real failure. Fixed, redeployed, re-verified via the actual `systemctl start` path before the fleet-wide widening. **Wazuh indexer gap closed 2026-09-08**: a real OpenSearch snapshot mechanism (filesystem repo, native snapshot API, `partial: true` to tolerate a handful of unrelated old dead indices) now backs up sn-security's security-event history nightly — verified via a real restore of a live alert index into a renamed copy, matching doc count and content, not just pushed. See `docs/HoT_PBS_Backup_Integration_Scope.md` Section 11 for the full build, including a real CrowdSec false-ban incident (`http-generic-403-bf` banning hot-bm-nl's shared public IP, breaking every VM's public health probes) surfaced and fixed along the way — **root cause corrected 2026-09-13, see below: this was NOT an unrelated stale `forgejo-runner` auth token** (that token was re-tested live and authenticates fine) — it recurred identically on 2026-09-13 with the runner stopped the whole time, proving the real mechanism is the self-perpetuating loop described in the Operational Rules table. Wazuh's *config* (rules/decoders/manager.conf) is covered via the `sync.sh` fix below. sn-web has no meaningful runtime state at all (checked live — under 200KB total across all 6 sites + the Stratus app), covered only via `sync.sh`. **hot-pn widened twice more since the first-run count above**: Immich's Postgres+library added 2026-09-09 (deploy day); Notesnook's Mongo+MinIO added 2026-09-10, closing a real gap found while investigating Inventory field-population bugs — this stack had existed since 2026-08-10 with zero application-consistent backup at all, only the non-Mongo-aware `pbs-host-backup.sh` raw tar. See services-hotpn.md's Immich section for the full trace of both. **`service_backups` integration added 2026-09-10, closing a second real gap Mr. Byrne caught** ("health checks and backup jobs still need updating and fixing"): the real backups above always genuinely ran, but nothing outside PN's own `pg_dump.sh` (`privatenexus-db` only) ever told PN's own governance/recovery tracking about it — 32 of 33 `backup_policy: daily`-labeled services had zero rows in `service_backups`, so PN's UI showed "Daily" right next to "never backed up". New `register_backup()` helper in this script, same direct-psql-on-hot-pn pattern `pg_dump.sh` already used (not through the app's session-gated API — this runs unattended with no session to hold), called after every successful dump/tar/snapshot in this script (17 call sites, some registering against both a DB sidecar row and the parent app row a person actually looks at — e.g. Nextcloud's data tar registers against `nextcloud`, not just `nextcloud-db`). Verified standalone against a real row before trusting it in the full script. **Real bug caught on the first full production run, 2026-09-11**: the `notesnook-s3` block had been silently backing up nothing since it was written the day before — MinIO's image has no `tar` binary at all, and `[ -s "$file" ]` doesn't catch a failed-command's-still-produces-a-tiny-nonempty-gzip-stream false positive. Fixed by tarring the host-side bind mount directly (no `docker exec` needed — MinIO's data is a bind mount, not a named volume) and raising the emptiness check to a real byte threshold. See `pn_health_and_backup_tracking_fix_2026_09_10` memory. |
| pbs-host-backup | systemd (`pbs-host-backup.timer`, on Gateway/hot-pn/hot-erp-nl) | Not a container — daily (03:40, `hot-config/scripts/pbs-host-backup.sh` + `hot-config/gateway/pbs-host-backup/`) `proxmox-backup-client` push straight to PBS (`pbs-hot`'s datastore, `houseoftrae-backups`), built 2026-09-07 at Mr. Byrne's explicit direction: PBS should be a continuously-updated, fully-encrypted local (non-cloud) copy of everything needed to rebuild HoT, not just the 3 Proxmox VMs it already covered. Closes the real gap that Gateway/hot-pn/hot-erp-nl are standalone VPS's, not Proxmox VMs — vzdump/`pbs-hot` can never reach them, so their real state had no local-storage backup at all before this (only cloud, via `fleet-state-backup.sh`/`backup-gateway-vps.sh`/hot-pn's own pg_dump chain). Backs up `/opt` (stacks, secrets, hot-config), `/root` (SSH keys, WireGuard client configs), `/var/lib/docker/volumes` (named-volume data that isn't bind-mounted under `/opt` — confirmed real and non-trivial on all 3 hosts before this was built, not assumed empty: 778MB/1.3GB/915MB respectively), and `/etc/wireguard` where present. Deliberately not a whole-OS backup — same "rebuild recipe" philosophy as `docs/HoT_PBS_Backup_Integration_Scope.md` Section 9 (OS/packages are reprovisionable, real state and secrets aren't). Reaches PBS via `wg6` directly from the Gateway (`10.10.5.2`) and via Tailscale from hot-pn/hot-erp-nl (PBS's own Tailscale IP, `100.126.193.54` — both paths were already confirmed reachable in the original PBS scope doc's connectivity table). Encrypted client-side with a dedicated scrypt-passphrase-protected key (`fleet-hosts@pbs!hostbackup`, a least-privilege PBS token scoped to `Datastore.Backup` only — cannot read/modify other backups), key+passphrase saved to Vaultwarden — **losing both makes every backup this script ever wrote permanently unrecoverable, this is not a soft dependency.** Verified end-to-end with real first-run backups on all 3 hosts before trusting the timer (Gateway 2.9GB/150s, hot-erp-nl 838MB/48s, hot-pn 13.5GB/54min — hot-pn's is the true cold-start baseline; PBS's own incremental dedup should shrink nightly runs sharply from here). **Same day, closes the parallel gap on the VM side**: `pbs-hot` storage itself had no client-side encryption key at all until this session (`pvesm set pbs-hot --encryption-key autogen` on hot-bm-nl) — that raw key was also pulled off hot-bm-nl and saved to Vaultwarden immediately, since PVE's own copy (`/etc/pve/priv/storage/pbs-hot.enc`) is the only other copy and isn't passphrase-protected (a PVE storage-integration limitation, not a choice). The `daily-fleet-backup-pbs` vzdump job (VM 102/104/106) was reverted from its 2026-09-04 interim `local-backup-zfs` fallback back to `pbs-hot`, now that PBS/wg6 are confirmed healthy and monitored (`wg6-handshake`/`pbs-hot-storage` checks, live since 09-06). A new job, `daily-fleet-backup-pbs-sninfra` (02:15), additionally sends VM 100/sn-infra to `pbs-hot` too — additive, not a replacement for its existing `local-backup-zfs`+Hetzner-offsite path, so sn-infra keeps its independent-of-PBS redundancy while still landing in PBS for full-fleet rebuild coverage. |
...
other sn-infra service. **Keycloak OIDC login confirmed working 2026-09-02** (real disposable test
account driven through the full multi-hop broker redirect chain via curl, deleted after — see
`wikijs_auto_admin_enrollment_2026_09_02` memory) — this also caught and fixed a real bug:
`hot-wiki` (and `docs.privatenexus.net`) were auto-enrolling every successful login as full
Administrator (`autoEnrollGroups` pointed at the built-in Administrators group); both repointed at
a proper limited group (`Editors` here, with page write access — this is an internal staff KB, not
customer-facing), re-verified live post-fix. Membership review came back clean — only Mr. Byrne's
own accounts had ever actually logged in on either wiki. **Content confirmed real, not a stub**: 18
published pages as of 2026-09-03 (Network & Access, Hardware & Capacity, Services Directory,
Operational Rules, Incident Response, DNS Zones, Security Tooling — mirrors `claude-md`'s own topic
structure), spot-checked one page's real body, not just metadata. Still no
Uptime Kuma/Gatus/UptimeRobot monitor exists for it — same class of gap as `docs.privatenexus.net`,
not yet closed. **Taken down 2026-09-02 by the same qm hard-cycle bug
...
**Agent enrollment — full fleet coverage as of 2026-08-08, extended to hot-edge-ch 2026-09-15.**
Until 2026-08-08 only the manager itself and the Gateway VPS had an agent installed;
sn-infra/sn-web/sn-monitor/hot-bm-nl/hot-pn/hot-erp-nl all lacked one. All 8 non-manager hosts
(agent IDs 001-008) are now enrolled and Active (`agent_control -l` on the manager container).
Install pattern: Wazuh's own apt repo (`packages.wazuh.com/4.x/apt stable main`, `wazuh.gpg`
keyring — minimal Debian VM images need `gnupg` installed first), `WAZUH_MANAGER=10.10.70.106
apt-get install -y wazuh-agent`, then `/var/ossec/bin/agent-auth -m 10.10.70.106 -A $(hostname)`
(talks to `wazuh-authd` on sn-security:1515, already running/exposed, no manager-side change
needed), `systemctl enable --now wazuh-agent`. hot-bm-nl's agent registered under its real hostname
`proxmox22272.hostkey.in`.

**hot-edge-ch (ID 008) enrolled 2026-09-15**, closing out the last open item from its build (see
[[hot_edge_ch_built_2026_09_15]]). Hit the exact same `wg syncconf`-doesn't-install-routes bug as
the sn-security routing regression found earlier the same session (see `operational-rules.md`) —
but the mirror image of it: this time the *forward* SYN path worked fine end-to-end (verified via
live packet counters on the Gateway's `wg7→wg4` UFW forward rules), but hot-bm-nl's own `wg0.conf`
had `10.10.6.2/32` (hot-edge-ch) correctly in its peer `AllowedIPs` — added back when the tunnel was
first built — with no matching kernel route ever installed, so sn-security's SYN-ACK response had
nowhere to go once it reached hot-bm-nl and was silently dropped. Fixed with
`ip route add 10.10.6.2/32 dev wg0` on hot-bm-nl — connection completed immediately after. Worth
remembering as a general pattern: this bug can bite on *either* end of a widened `AllowedIPs` pair,
not just the end that was most recently edited — check both sides' routing tables, not just the one
you touched.
