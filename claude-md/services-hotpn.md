...
**Public domain migrated to privatenexus.net, 2026-09-09.** Nextcloud (`cloud.house-of-trae.com`)
and all 4 of Notesnook's public-facing services (`notes-{auth,sync,sse,s3}.house-of-trae.com`) moved
to `cloud.privatenexus.net`/`notes-{auth,sync,sse,s3}.privatenexus.net` as their canonical URLs —
see CLAUDE.md's Domain Assignment Policy rule 3 for the decision (Mr. Byrne's call: personal services
now deliberately live under the PrivateNexus banner, to showcase the Catalogue automation actually
running them). Old `house-of-trae.com` URLs kept as permanent 301s, same pattern as
`monitor.securenexus.net`→`monitor.house-of-trae.com`. Nextcloud's `trusted_domains` needed a live
`occ config:system:set` fix post-migration (its Docker-secret-bootstrapped config only ever knew the
old domain) — worth checking on any future Nextcloud domain change, this isn't automatic. Any device
syncing Nextcloud via WebDAV/CalDAV needs manually re-pointed at the new URL — a redirect doesn't
carry a sync client across a domain change. Real WebDAV/CalDAV-can't-redirect and native-app-auth
exceptions on the `import sso` question both carried forward unchanged onto the new domain names.
...
| pn-docs | /opt/stacks/pn-docs/ | **docs.privatenexus.net — deployed and live 2026-08-25**, PrivateNexus's customer-facing product documentation/knowledge base, prioritized ahead of the HoT-internal `wiki.house-of-trae.com` item (still an open 502) given the upcoming release. `requarks/wiki:2` (Wiki.js) + its own Postgres 16 container (`pn-docs-db`), bound to `10.10.2.2:3080` (tunnel-scoped, matching every other PN service — never `0.0.0.0`), UFW-scoped to `10.10.2.1` only. DB password via a Docker secret + a small `entrypoint.sh` wrapper (same pattern as Keycloak's own — Wiki.js has no native `_FILE`-suffix env support). Caddy site block deliberately has **no `import sso`** — this is public-readable customer documentation, not admin tooling, matching the same documented exception as `cloud.house-of-trae.com` (New Service Deployment Checklist item 13); editor/admin access is gated separately, inside Wiki.js itself, via a dedicated `pn-docs` OIDC client registered in the `privatenexus` Keycloak realm (redirect URI `https://docs.privatenexus.net/login/*`, secret in Vaultwarden). **Mr. Byrne completed the first-run setup wizard 2026-08-25** (Site URL `https://docs.privatenexus.net`, admin account `tristian@securenexus.net`, telemetry off) — confirmed live from this side too (`200`, real Wiki.js welcome page, setup wizard gone). **Keycloak OIDC login wired up and verified working end-to-end the same day**, after real debugging — two genuine bugs found, not one: (1) the OIDC strategy's Authorization Endpoint URL had a stray leading space, and `displayNameClaim` was set to `"displayName"` (not a real Keycloak claim — should be `"name"`), both fixed but **neither was actually the cause of the 500** being hit; (2) the real cause, found only after temporarily setting `NODE_ENV=development` to get Wiki.js's error page to show a stack trace (its global error handler never logs to the server console at any `LOG_LEVEL` — confirmed by reading `master.js` directly, `res.render('error', { message: err.message, error: WIKI.IS_DEBUG ? err : {} })`, no logger call at all): `User.processProfile()` in `server/models/users.js` throws `"You are not authorized to login."` when a brand-new OIDC identity has no matching local account and the strategy's **Self-Registration** toggle is off. Fixed by enabling Self-Registration + setting Auto-Enroll Groups on the strategy. Verified via a real login: a new `users` row appeared (`tristian@securenexus.net`, provider-linked, `isActive: true`) and the callback returned a clean `302` instead of `500`. Both temporary debug env vars (`LOG_LEVEL=debug`, `NODE_ENV=development`) were reverted after diagnosis — production config only carries the DB connection vars. **UptimeRobot monitor added 2026-09-16** (HTTP(s), `https://docs.privatenexus.net`, ID `804006498`, confirmed `up`) — closes the follow-up flagged here. Uptime Kuma itself still has no monitor for this (Socket.IO-driven, no simple REST API) — not pursued further since UptimeRobot now covers the same independent-external-monitoring need. The UptimeRobot MCP server's own `newMonitor` call is still API-blocked on the free plan (confirmed live 2026-09-16, see `hot_edge_ch_sn_security_routing_regression_2026_09_15` memory) — this one was added manually in the dashboard, same as `hot-edge-ch`'s. |
...
| Immich | /opt/stacks/immich/ | **photos.privatenexus.net — deployed 2026-09-09** via the Catalogue flow (Phase 3 of the pattern proven twice already with Nextcloud/Notesnook), closing out the last unresolved piece of the HoT Sync roadmap item (see `hot_sync_landing_spot_rescoped_2026_09_04` memory — Vaultwarden/Nextcloud/Notesnook all already had homes, Immich didn't). New template authored at `catalogue/templates/immich/docker-compose.yml.tmpl` (the catalog's `immich` entry existed with no `deploy` block until this session) — `immich-server` + `immich-machine-learning` + `ghcr.io/immich-app/postgres` (vectorchord/pgvector bundle) + `redis:7-alpine`, all container names/image tags pulled live from `docs.immich.app` rather than guessed. Bound directly to `10.10.2.2:26286` (auto-assigned host port) from the start — deliberately skipping the 127.0.0.1→tunnel-IP drift-then-formalize cycle Nextcloud/Notesnook went through (see `pn_nextcloud_binding_and_personal_cleanup_2026_08_20`); confirmed zero drift via `catalogue-drift-check.py` post-deploy. No `import sso` on the Caddy block — native Keycloak OIDC instead (client `immich`, `privatenexus` realm, confidential, redirect URIs per `docs.immich.app`'s OAuth doc — secret in Vaultwarden's PrivateNexus folder). **Two things need doing once Mr. Byrne completes Immich's own first-run admin signup** (no env-based bootstrap exists for this app, unlike Nextcloud): (1) Administration → Settings → Machine Learning → URL = `http://immich-ml:3003` (the ML container's real name on the shared `pn-catalogue-deployments` network — Immich's built-in default of `immich-machine-learning` won't resolve here); (2) Administration → Settings → Authentication → OAuth — Issuer `https://auth.house-of-trae.com/realms/privatenexus`, Client ID `immich`, the Vaultwarden secret, Auto Register on. `health_endpoint` (`http://10.10.2.2:26286/`) was auto-populated correctly by the executor at deploy time — the Nextcloud/Notesnook-era gap where this had to be fixed post-hoc (`pn_health_endpoint_gap_2026_08_19`) is closed generically now. Backed up nightly via `fleet-state-backup.sh` (Postgres dump + real library directory, same treatment as Nextcloud's). Uptime Kuma (`[Internal] Immich`) and Gatus (`Personal Services (privatenexus.net)` group) both added the same day. Test upload/delete cycle run via the `jarvis` API key (Mr. Byrne's own, created for PN automation, saved to Vaultwarden) 2026-09-10 — 2 images uploaded, verified byte-exact via `exifInfo.fileSizeInByte` + quota delta, then deleted; library confirmed empty and working end-to-end. |

**⚠ Real bug found+fixed 2026-09-10: the Catalogue executor's `services` INSERT silently dropped or hardcoded several fields, discovered when Mr. Byrne reviewed Immich's row in Inventory and found backup policy, access URL, workspace, and recovery runbook all wrong/blank.** Root cause, `routes/actions.js`'s `service.provision_from_catalogue` handler: `backup_policy` was hardcoded to the literal string `'none'` regardless of what the catalog entry declared (every app on offer — Nextcloud/Notesnook/Immich — says `"backup": "daily"`); `access_url`, `workspace_id`, and `recovery_runbook_url` weren't in the INSERT's column list at all, always NULL; `container_name` was blindly set to the deploy slug, correct for single-primary-container templates (Nextcloud/Immich, whose main container really is named exactly the slug) but wrong for Notesnook (no container is ever bare `notesnook` — real containers are `notesnook-identity`/`-sync`/etc.). **Fixed for all future deploys**: `routes/catalogue.js`'s preview route now forwards `app_backup` and a derived `app_access_url` (`https://{subdomain}.privatenexus.net`, following the 2026-09-09 domain banner decision) into the params the frontend already spreads straight into `propose`; `executeProvisionFromCatalogue` now tracks and returns the actual container backing the resolved health endpoint (`primaryContainerName`) instead of assuming the slug; the INSERT looks up the real `Personal Services` workspace (by slug, not a hardcoded UUID) and uses all of the above. Backend rebuilt+recreated same session. **The 3 existing rows (Nextcloud/Notesnook/Immich) shipped before this fix and still need a one-time manual correction via the Inventory Edit UI** — this session couldn't apply it directly (writing to `services` outside the app's own session-authenticated path is the same category of action a prior session's direct-SQL attempt was correctly classifier-blocked on, see `pn_health_endpoint_gap_2026_08_19`/`hot_sync_landing_spot_rescoped_2026_09_04` memories) — see `personal_services_field_population_bug_2026_09_10` memory for the exact corrected values to paste in for each row. Real runbook docs written the same session (`docs/runbooks/{nextcloud,notesnook,immich}.md` on hot-pn, following the existing format) so the new `recovery_runbook_url` convention link actually resolves instead of 404ing — will reach the public GitHub mirror on `pn-config-sync.sh`'s next nightly run, not instantly. **Separately found and fixed the same session**: Notesnook had zero application-consistent backup coverage at all since its 2026-08-10 deploy (only `pbs-host-backup.sh`'s non-Mongo-aware raw `/opt` tar) — added a real `mongodump`+MinIO-attachments section to `fleet-state-backup.sh`, verified standalone (confirmed via `mongosh` that no `notesnook` database even exists yet — zero real users, matches prior findings — so this was previously an unexercised gap, not a live data-loss risk, but a real one going forward).
