...
**⚠ Recurred 2026-09-04 despite the WAL fix above still being in place — root-caused and permanently fixed by migrating off SQLite entirely, not by tuning WAL further.** A real burst of 1,280 `SQLITE_BUSY` errors ran ~00:00–00:29 UTC, including a genuine 7.5s `500` on the Grafana login page and 3 CRITICAL SMS texts (`FIRING: DatasourceError — node-exporter is unreachable`) to Mr. Byrne's phone for an alert that was never really down (confirmed live: all 6 fleet node-exporters reachable, all 28 central Prometheus targets `up`). **A real gap found alongside it**: unlike the equivalent 2026-08-09/09-03 incidents, no `RESOLVED` SMS ever went out this time even though the alert genuinely cleared by 00:30 — most likely the same lock contention that was blocking `ngalert.state.manager.persist` also interfered with the resolve-notification path itself. `GF_DATABASE_WAL: "true"` was confirmed still live in both the compose file and the running container — not reverted, just insufficient under whatever write-concurrency spike hit that night (WAL removes the *reader-blocks-writer* case but SQLite still serializes writers, and Grafana's newer "app platform"/unified-storage job-driver subsystem, visible in the logs as `resource-server`/`provisioning.grafana.app`/`job-cleanup-controller` errors, adds real concurrent write load beyond just alert-rule evaluation).

**Fixed properly, same day: migrated Grafana off bundled SQLite to a dedicated `grafana-db` Postgres 16 container** on the same `monitoring` bridge network (`/opt/stacks/monitoring/docker-compose.yml` — `GF_DATABASE_TYPE=postgres`, password via Docker secret, `pg_isready` healthcheck gating Grafana's `depends_on`). Real dataset check before choosing migration approach: **0 dashboards**, only 6 alert rules / 2 datasources / 2 users / 5,617 annotation rows (alert-state history, not displayed anywhere since there are no dashboards) — small enough that a full binary SQLite→Postgres migration (e.g. `pgloader`) wasn't worth the cross-engine schema risk. Instead: exported every real object via the provisioning API first (alert rules, contact points, notification policy tree, folder, datasources — all saved as a safety net), cut Grafana over to a fresh empty Postgres (which runs Grafana's own **correct, native** Postgres migrations — zero schema-translation risk), then re-created all 6 alert rules + both contact points (`email-hot`, `ntfy-critical` — real Ntfy bearer token pulled from `gateway/crowdsec/notifications/http.yaml`, GET responses mask it as `[REDACTED]` so it isn't recoverable from the export alone) + the notification policy tree + the `HoT Infrastructure Alerts` folder + both datasources via the API, **deliberately preserving every original UID** so the alert rules' `folderUID`/`datasourceUid` references needed zero rewriting. Verified post-migration: all 6 rules present with `execErrState: Error` intact, both datasources/contact points matching, zero active/errored alerts, dashboard count still 0 (nothing lost that mattered), Grafana footprint unchanged (~440MB) and `grafana-db` itself only ~55MB RSS — a non-issue against sn-monitor's tight RAM (see the note on fleet-wide KVM ballooning below). **Accepted, explicit trade-off**: the 5,617 rows of annotation history and per-user UI preferences were not migrated (users just re-authenticate via Keycloak OAuth, which recreates their account automatically) — both original `grafana.db` and a timestamped `.bak` were left in place at `/opt/monitoring/grafana/data/` as an inert rollback safety net, no longer read by Grafana. `grafana-db` password in Vaultwarden ("sn-monitor Grafana Postgres DB password (grafana-db container)", House of Trae — Gateway VPS folder).

**Separate, real finding surfaced while investigating this, not yet acted on**: all 4 hot-bm-nl VMs were actively KVM-balloon-squeezed below their configured RAM at the time (e.g. sn-monitor showing 2.8GB visible against a configured 4GB, `free_mem` in the 100-130MB range, and genuine guest-level swapping on every VM, sn-security swapping over 1GB) — same symptom class as the 2026-08-08 ZFS ARC/ballooning incident, but this time ARC is holding correctly at its 8GB cap (not blown past it), so the renewed pressure is coming from the sum of all 4 VMs' footprint plus ARC now filling most of hot-bm-nl's 31GB. Host itself wasn't in crisis (3.8GB free / 4.1GB available), and all 4 VM processes show a `Sep02` start time, meaning this coincides with the 4 VMs being hard-power-cycled back up during that day's outage (`reboot_watchdog_proxmox_failure_outage_2026_09_02` memory) — plausibly a genuine contributing factor to this same night's SQLite lock storm (swap-induced disk I/O latency compounding SQLite's lock timeouts), not confirmed. Worth a fleet-wide memory headroom check if this recurs on another VM.
