...
### ⚠️ Undocumented container found on the hot-bm-nl host itself, 2026-08-19
`fleet-judgment-review`'s first run with PN's Discovery queue folded in (Option D2 → D1, see
`services-fleet.md`) flagged a container running directly on hot-bm-nl (not inside one of its 4
VMs) that had never been fleet-discovery-pushed or documented anywhere. Verified live, not just
relayed from the LLM: a real `jonasal/nginx-certbot:latest` container (`nginx-nginx-1`, `docker ps`
shows `Up 2 weeks`, created ~2026-07-23 per `/root/nginx/compose.yml`'s mtime — **predates this
project's own 2026-07-27 hot-bm-nl rebuild**), `network_mode: host`, reverse-proxying HTTPS on 443
to the local Proxmox UI (`proxy_pass https://127.0.0.1:8006`) under the hostname
`proxmox22272.hostkey.in` with a real Let's Encrypt cert (`CERTBOT_EMAIL=hkadm@hostkey.com`).

That email domain and the pre-rebuild creation date strongly suggest this is **Hostkey's own
provisioning/support tooling for this oVirt VPS**, not something House of Trae's team or any prior
Claude Code session built — not confirmed with Hostkey directly, but the evidence points that way,
not at a compromise. **Checked whether this violates the "bare metal has zero public-facing ports"
rule**: it does not, in practice — `ufw status verbose` on hot-bm-nl shows `Default: deny
(incoming)` with no `443` allow rule, and a real external connection attempt to hot-bm-nl's public
IP (`31.207.47.146:443`) from the Gateway timed out. Host-networking containers bypass Docker's own
NAT (unlike the `-p`-published-port case documented in the hot-pn section, where Docker's iptables
rules are known to bypass UFW) — but they don't bypass UFW's own INPUT chain, so this one is
correctly firewalled off from the public internet despite listening on the host's real interface.

**Not removed or touched** — pending Mr. Byrne's confirmation of what this actually is (Hostkey
support access vs. something else) before anyone decides whether to leave it, document it as
permanent, or ask Hostkey about it. Real gap this exposed: `fleet-health-sweep`'s stacks-inventory
check only covers each VM's `/opt/stacks/`, never hot-bm-nl's own root-level Docker containers — so
this had zero chance of being caught by the deterministic nightly sweep (Option A), only by D1's
judgment pass noticing it in the Discovery queue's raw candidate list. Worth considering as a future
`fleet-health-sweep` scope addition if this pattern recurs.
