...
**⚠ Real throughput finding, 2026-09-07: PBS write speed for a full VM backup measured ~4 MiB/s (VM 104/sn-monitor, first encrypted backup), not the "fast download" side of PBS's asymmetric home link the earlier scope doc's numbers implied.** At that rate a full 250GB VM image (sn-web is the largest) could take 15-18h — worth watching the first few real nightly runs of `daily-fleet-backup-pbs` for pileup (a run still going when the next night's job starts) before treating this as settled; PBS's own chunk-dedup should shrink this sharply after the first backup per VM, same pattern already seen on the new `pbs-host-backup.sh` (hot-pn's cold-start 13.5GB run vs. what nightly incrementals should look like from here). See `docs/HoT_PBS_Backup_Integration_Scope.md` Section 10 and `services-fleet.md`'s `pbs-host-backup` row for the full build.
