...
|----------------|-----------------------|-------------|-------------------------|------------------------------------|
| VM snapshots, PBS (sn-web/sn-monitor/sn-security + sn-infra, additive) | Proxmox → PBS (`pbs-hot` storage, datastore `houseoftrae-backups`, over the `wg6` tunnel) | 02:00 daily (100 offset to 02:15, its own job) | PBS — Mr. Byrne's own local hardware, single disk, no RAID | **PBS client-side encryption, enabled 2026-09-07** (`pvesm set pbs-hot --encryption-key autogen` on hot-bm-nl) — was unencrypted-at-rest before this, see note below |
| VM snapshots (sn-infra only, redundant path) | vzdump (Proxmox) | 02:00 daily | sn-infra (100): `local-zfs` (`local-backup-zfs` storage) — kept alongside the new PBS job above, not replaced, so sn-infra stays covered even if PBS/wg6 is down | zstd compressed |
| VM snapshots offsite (hot-bm-nl) | rclone crypt (`vzdump-offsite-push.sh`, systemd timer) | 04:30 daily (after the 02:00 vzdump job) | **Hetzner Storage Box only** — covers the `local-zfs`-targeted job above; PBS-targeted backups have no offsite cloud copy (by design — PBS is now the local-storage rebuild target Mr. Byrne asked for, not a cloud-backup source) | rclone crypt (hetzner-crypt remote) |
| Host-level rebuild backup (Gateway, hot-pn, hot-erp-nl) | `pbs-host-backup.sh` (systemd timer, all 3 hosts) — `proxmox-backup-client` push of `/opt`, `/root`, `/var/lib/docker/volumes`, `/etc/wireguard` | 03:40 daily | PBS — same local hardware/datastore as the VM backups above, reached via `wg6` (Gateway) or Tailscale (hot-pn/hot-erp-nl) | Client-side AES-256-GCM, scrypt-passphrase key (`fleet-hosts@pbs!hostbackup`) — key+passphrase in Vaultwarden |
...
| Fleet state (Vaultwarden, Grafana, Uptime Kuma, Forgejo, PowerDNS-Admin, hot-wiki, Namevault, ERPNext, Nextcloud DBs + real data) | `fleet-state-backup.sh` (Gateway, systemd timer) — pg_dump/mariadb-dump/`sqlite3 .backup`/`tar` over SSH, direct push, no hot-bm-nl/PBS involved | 03:10 daily | **Hetzner + B2, both attempted every night unconditionally** | rclone crypt (hetzner-crypt/b2-hot-crypt) |
...
**Throughput finding from 2026-09-07, resolved same day**: VM 104's very first encrypted PBS backup showed ~4 MiB/s mid-transfer, which looked concerning (15-18h projected for a full 250GB image). A clean re-run confirmed this was a non-issue: PBS's chunk-based dedup reused 99% of the VM's data from a pre-outage backup already on the datastore, so the real transfer was 1.12GB in 169s (6.8 MiB/s) — full nightly runs going forward should look like this, not the cold-start number. Same pattern confirmed on `pbs-host-backup.sh`: hot-pn's cold-start run was 13.5GB/54min, its very next run was 49s.

**⚠ Real bug found+fixed the same day: both PBS backup paths' retention prune step was failing on every run with `missing Datastore.Modify|Datastore.Prune`, even though the backup itself succeeded — this failed the whole vzdump job nightly (mail-to-root "job errors") despite real data landing safely.** Root cause: `DatastoreBackup` (the role originally granted to both `hot-bm-nl@pbs!vzdump` and the new `fleet-hosts@pbs!hostbackup` tokens) doesn't include prune privilege. Granting `DatastorePowerUser` to the *token* alone wasn't sufficient either — PBS tokens are privilege-separated by default, so the effective permission is the intersection of the token's own ACL and its underlying user's ACL; the fix needed `DatastorePowerUser` granted to both `fleet-hosts@pbs` (user) and `fleet-hosts@pbs!hostbackup` (token), not just one. Verified clean via real end-to-end reruns on all 4 backup paths (VM 104's full vzdump job, plus Gateway/hot-pn/hot-erp-nl's host-level script) — check this exact intersection rule first if a future PBS token's operation fails despite what looks like the right ACL grant.
