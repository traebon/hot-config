...
- HoT Sync (Flutter) — Immich + Nextcloud + Notesnook + Vaultwarden; no longer tied to sn-personal
  (VLAN 40 retired outright 2026-08-09, see the sn-personal section above). **Infrastructure
  question fully closed 2026-09-09** — all 4 target apps now have real homes: Vaultwarden stays on
  the Gateway (its own documented reason, unrelated to this question); Nextcloud/Notesnook/Immich
  all live on hot-pn via the Catalogue deploy flow, all under the `privatenexus.net` banner as of
  the same-day domain consolidation (see CLAUDE.md's Domain Assignment Policy rule 3 and
  services-hotpn.md's Immich entry). Immich was the one genuinely missing piece (per
  `hot_sync_landing_spot_rescoped_2026_09_04`'s finding) — built fresh, deployed, verified, backed
  up, and monitored the same day. **What's left is adoption/usage, not infrastructure**: Notesnook
  still has zero real users, and Immich has no photos yet (Mr. Byrne needs to complete its first-run
  admin signup — see the two pending Immich settings noted in services-hotpn.md). The Flutter mobile
  app itself (the actual "HoT Sync" client) was never started — this roadmap item was always about
