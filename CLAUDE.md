...
| node-exporter | /opt/stacks/node-exporter/ | Deployed 2026-08-08 (was previously the one host with zero memory/host metrics — investigating an SMS alert storm surfaced this gap, see `sms_alert_storm_investigation_2026_08_08` memory). Standard host-network compose, port 9100, not exposed publicly (no UFW rule). **Not wired into sn-monitor's central Prometheus** — hot-erp-nl sits behind its own `wg5` tunnel with no routing to sn-monitor's `wg4`/VLAN mesh (confirmed live: neither host can ping the other), the same reason hot-pn runs its own local Prometheus instead of being centrally scraped. Making it centrally reachable would need wider WireGuard `AllowedIPs` + routing across the Gateway/hot-bm-nl/hot-erp-nl plus UFW forwarding rules — bigger surgery than deploying the exporter itself, not done yet. For now, query it directly (`ssh hot-erp-nl curl localhost:9100/metrics`) rather than via Grafana. |
